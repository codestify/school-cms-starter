...
<!-- banner -->
<?php
$select = "SELECT * FROM news WHERE news_title='". $title ."' LIMIT 1";
$query = mysqli_query($dbconnect, $select);
$row = mysqli_fetch_array($query);
?>
<section class="academic-banner" style="background-image: url('/storage/admin/news/<?=$row['news_banner_image']; ?>');">
  <div class="about-opacity">
     <div class="container">
         <h2><?= $row['news_title']; ?></h2>
         <p><?= $row['news_subtitle']; ?></p>
     </div> 
  </div>
</section>
<!-- / banner -->
...
<!-- admission page content -->
    <div class="col-sm-10 col-sm-offset-1">
      <article>
        ...
        <p><img src="/storage/admin/news/<?= $row['news_image'] ?>" class="img-responsive" alt="about image"></p>
          <?= $row['news_details']; ?>
      </article>
    </div>
<!-- admission page content -->
